...
% Code to generate reasonable filter bank
    ...
    prewittScales  = [1 2 4 8 sqrt(2)*8];
    

    filterBank = cell(numel(gaussianScales) + numel(logScales) + numel(prewittScales) + numel(prewittScales),1);
    ...
    for scale = prewittScales
        idx = idx + 1;
        filterBank{idx} = fspecial('prewitt');
    end
